    ...
    <div style={{ maxWidth: 720 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div>
            ...
            <div style={{ border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '110px 60px 1fr 1fr', background: 'var(--surface2)', padding: '7px 12px', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-muted)' }}>
                <span>Day</span><span>Open</span><span>From</span><span>Until</span>
              </div>
              {DAYS.map((day, i) => {
                const sched = availability[day]
                return (
                  <div key={day} style={{ display: 'grid', gridTemplateColumns: '110px 60px 1fr 1fr', alignItems: 'center', padding: '8px 12px', borderTop: i > 0 ? '1px solid var(--border)' : 'none', background: sched.open ? 'var(--surface)' : 'var(--surface2)' }}>
                    ...
                    {sched.open ? (
                      <>
                        <input className="form-input" type="time" value={sched.from} onChange={e => setDay(day, 'from', e.target.value)} style={{ margin: '0 8px 0 0', padding: '5px 8px', fontSize: 12 }} />
                        <input className="form-input" type="time" value={sched.until} onChange={e => setDay(day, 'until', e.target.value)} style={{ padding: '5px 8px', fontSize: 12 }} />
                      </>
                    ) : (
      ...
      {activeSection === 'integrations' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div>
            <div className="sec-label" style={{ marginBottom: 12 }}>Contact & Reviews</div>
            <div className="form-grid-2">
              <div>
          ...
          <div>
            <div className="sec-label" style={{ marginBottom: 4 }}>Stripe</div>
            <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 12 }}>
              Leave blank to use the global Stripe account configured in environment variables.
            </p>
            <div className="form-grid-2">
              <div>
          ...
          <div>
            <div className="sec-label" style={{ marginBottom: 4 }}>QuickFile</div>
            <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 12 }}>
              Leave blank to use the global QuickFile account configured in environment variables.
            </p>
            <div className="form-grid-3">
              <div>
      ...
      {activeSection === 'booking-link' && (
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 16 }}>Share this link with customers so they can request a booking for {currentSite.name}.</p>
          ...
          <div style={{ marginTop: 16 }}>
            <div className="sec-label" style={{ marginBottom: 8 }}>Public Availability Calendar</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--surface2)', borderRadius: 8, padding: '10px 12px' }}>
              <span style={{ flex: 1, fontSize: 12, fontFamily: 'monospace', color: 'var(--accent-text)', wordBreak: 'break-all' }}>
                {`${window.location.origin}/availability/${toSlug(form.name || currentSite.name)}`}
              </span>
              <button className="btn btn-ghost btn-sm" style={{ flexShrink: 0 }}
                onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/availability/${toSlug(form.name || currentSite.name)}`); setCopied(true); setTimeout(() => setCopied(false), 2000) }}>
                Copy
              </button>
            </div>
          </div>
      ...
      <div style={{ marginTop: 24, display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        {saved && <span style={{ fontSize: 12, color: 'var(--success, #16a34a)' }}>✓ Saved</span>}
